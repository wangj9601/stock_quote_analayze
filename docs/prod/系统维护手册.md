...
| RPE_STRATEGY_IMPLEMENTATION_DESIGN.md | 比价效应策略设计（参数、规则、表结构、API） |
| RPE_比价效应_信号计算规则.md | 比价效应信号流程、名词、分子/分母与判定步骤 |
| RPE_比价效应_业务简化版.md | 比价效应业务速览（含名词速查） |
| RPE_流动性过滤_分层改造方案.md | 流动性按上市板别分层均额（人民币元）+ 换手 |
