...
URT 回测目前 **仅 A 股**，无港股 / ETF 分支；类型为「交易向」模拟（含目标命中与风控离场），无单独的「仅命中率」模式。
...
## 3. 持仓观察与目标判定（对齐 GMS 命中率）

模式：`signal_hit_rate`（不止损）。观察窗口为信号日之后共 **`horizon_days` 根交易日 K 线**（默认 **20**，首根为入场日）。
