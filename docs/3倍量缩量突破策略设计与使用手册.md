...
- **VSB 选股观察股表**：`vsb_observe_stocks`（模型 `VsbObserveStock`）。在 `persist=true`（默认）且本表写入成功后，由同一 `save_screen_hits` 流程 **upsert**（唯一键 `(market, code, signal_date)`，默认 `market=CN`）；与 `triple_volume_observe_stocks`（日终爆量扫描）为不同业务表。建表：项目根目录执行 `python migrations/add_vsb_observe_stocks_table.py`。关闭自动写入：环境变量 **`VSB_SCREENING_OBSERVE_ENABLED=false`**。
...
- `GET /api/screening/volume-shrink-breakout-strategy` 在计算完成后，当 **`persist=true`（默认）** 且存在命中行时，由 [`signal_storage.save_screen_hits`](e:\wangxw\股票分析软件\编码\stock_quote_analayze\backend_core\strategies\volume_shrink_breakout\signal_storage.py) 批量提交 **`volume_shrink_breakout_signals`**，并在成功后默认同步 **`vsb_observe_stocks`**；任一落库失败 **只记日志，不改变选股 HTTP 成功结果**。
