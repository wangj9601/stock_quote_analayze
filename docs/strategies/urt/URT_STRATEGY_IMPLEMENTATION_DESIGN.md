...
| 连阳强度 | 最高 **40** | 见下表 |
| 量能倍数 | 最高约 **34** | 相对配置阈值缩放后的主量能分 |
| 中期阳线 | 最高约 **6** | 10/15/20 日相对阈值完成度等权 |
| 均线多头 | 最高约 **4** | `MA5>MA10>MA20`（`ma_bull_periods` 可配） |
...
先按原公式算满 40 分档，再 ×34/40 写入总分（为中期阳线/多头腾出空间）。

| 情形 | 公式（缩放前） |
|------|------|
| `vm >= need` | `30 + min(10, (vm - need) / need × 10)` |
| `vm < need` | `vm / need × 30` |
...
| 开关 | 规则 |
|------|------|
| `use_turnover` | 换手率按 **0%～8%** 线性映射到 **0～5** 分；开启时亦作硬筛 |
| `use_volume_ratio` | 量比按 **0～3** 线性映射到 **0～5** 分；开启时亦作硬筛 |
| `use_yang_medium` | 中期阳线始终计最多约 6 分；`true` 时硬筛须满足 `yang_medium_rules`（默认 10≥6、15≥8、20≥10） |
| `require_ma_bull` | 多头始终计最多约 4 分；`true` 时硬筛须 `MA5>MA10>MA20`（`ma_bull_periods`） |

默认配置下（换手/量比/中期硬筛/多头硬筛未启用）典型结构约为：MA20(10) + 连阳(≤40) + 量能(≤34) + 中期(≤6) + 多头(≤4)。
...
| 结构盈亏比提示 | `structure.rr` / `rr_reason` + `risk_tags`（id=`poor_structure_rr`）：RR=(阻力−收盘)/(收盘−支撑)；偏低或破位时打 warn/danger 标签；配置 `structure_rr_warn_enabled`（默认 true）、`structure_rr_min_rr`（默认 1.5）；**不改得分、不硬筛** |
...
| `ma_bull_periods` | 多头均线周期 | `[5,10,20]` | list | 严格递减即为多头 |
| `require_ma_bull` | 多头是否硬筛 | `false` | bool | `true` 时须多头排列 |

最少 K 线根数约：`max(ma_period, volume_lookback+1, yang 短窗, 中期最大窗, 多头最大周期)`。
...
| `structure_rr_min_rr` | 最低可接受 RR | `1.5` | RR=(阻力−收盘)/(收盘−支撑)；破位/贴阻力为 danger，偏低为 warn |
