...
| 连阳强度 | 最高 **40** | 见下表 |
| 量能倍数 | 最高约 **31** | 相对配置阈值缩放后的主量能分（原 34，腾出换手权重） |
| 中期阳线 | 最高约 **6** | 10/15/20 日相对阈值完成度等权 |
| 均线多头/空头 | **+0～10 / −8** | 前缀链深度分档（见下）；空头 −8 |
...
| `require_ma_bull` | 积分按前缀深度分档（满分默认 10）；`true` 时硬筛仍须 `MA5>MA10>MA20`（`ma_bull_periods`，不加长） |

**均线多头前缀分档**（`ma_bull_score_periods` 默认 5/10/20/30/60/120/250）：
...
默认配置下典型结构约为：MA20(10) + 连阳(≤40) + 量能(≤31) + 中期(≤6) + 多头(≤10) + 换手(±8)。
...
| `ma_bull_score_max` / `ma_bull_score_table` | 满分与深度→分表 | `10` / `[0,2,4,6,8,9,10]` | | d=2（短多）约 +4；d=6 全链 +10 |
