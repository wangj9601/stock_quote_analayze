...
主链路：日线 K 线现算指标 → 硬筛 → 百分制得分 → **结构硬闸（混合）** → 正式买点；KDE 支撑/阻力与结构盈亏比风险提示（RR 偏低为软标签）。
...
**结构硬闸（混合，默认开）**：KDE 有效时，**破位支撑 / 贴·超阻力 / 悬空离支撑**（相对支撑距离 ≥ 默认 **8%**）否决正式买点；**RR 偏低仅软标签，不否决**。KDE 无效不硬闸。
...
实现：`backend_core/strategies/urt/scoring.py`。
