...
个股主板块规则：优先**同花顺行业**（`board_code_source=tonghuashun`）；同 kind 多板块取成分最多（并列 `board_code` 升序）；无同花顺行业时回退同花顺概念。追溯重算全程只用该主板块。显式多选板块撞车时仍可按 **|Z| 最大** 去重。列表「板块」展示与选板一致；无同花顺行业映射时回退基础信息行业并标注，再无则 `--`。
...
| SBBR（做小做底） | 中小市值筑底转强（默认总市值约 20~200 亿 + 流通股约 5~10 亿股）；与 RPE 互补、互不依赖 |
