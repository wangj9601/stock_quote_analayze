#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""批量全市场 RS Rating（股价相对强度）预计算。

按交易日逐日重算 A 股截面排名并写入 ``rs_ratings``（前复权口径）。
缺省：以 ``historical_quotes`` 最新交易日为终点，向前约一年（日历 365 天）。
允许 ``--start`` / ``--end`` 指定区间。
...
用法（在项目根目录）::
    ...
    python scripts/batch_rs_rating_precompute.py --days 90
...
logger = logging.getLogger("batch_rs_rating")
...
def resolve_range(
    *,
    ...
    from backend_core.indicators.rs_rating.scheduled_precompute import resolve_trade_date

    db = SessionLocal()
    try:
        end_s = end or resolve_trade_date(db, None)
        if start:
            start_s = start
        else:
...
def run_batch(
    ...
    *,
    ...
    t_all = time.time()
    for i, d in enumerate(trade_dates, 1):
        logger.info("==== [%d/%d] 全市场 RS 预计算 %s ====", i, total, d)
        t0 = time.time()
        try:
            summary = run_rs_rating_precompute(trade_date=d)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="批量全市场 RS Rating 预计算（缺省最近一年交易日）",
    )
    parser.add_argument(
        "--start",
        type=_parse_ymd,
        default=None,
        help="起始日 YYYY-MM-DD（含）；缺省=结束日往前 --days 天",
    )
    parser.add_argument(
        "--end",
        type=_parse_ymd,
        default=None,
        help="结束日 YYYY-MM-DD（含）；缺省=historical_quotes 最新交易日",
    )
    parser.add_argument(
        "--days",
        type=int,
        default=365,
        help="未指定 --start 时，自结束日向前的日历天数（默认 365≈一年）",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="只列出将要计算的交易日，不落库",
    )
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
    ...
    if not dates:
        logger.error("区间 %s ~ %s 在 historical_quotes 中无交易日", start_s, end_s)
        return 2

    logger.info(
        "区间 %s ~ %s，共 %d 个交易日（全市场截面；耗时可能很长）",
        start_s,
        end_s,
        len(dates),
    )
