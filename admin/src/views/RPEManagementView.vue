<template>
  <div class="rpe-management">
    <div class="page-header">
      <h1 class="page-title">比价效应（RPE）管理</h1>
    ...
    <el-dialog v-model="showBt" title="创建回测" width="560px">
      <el-form label-width="140px">
        ...
        <el-form-item label="目标相对收益">
          <el-input-number v-model="btForm.target_relative_pct" :min="0.01" :max="1" :step="0.01" />
        </el-form-item>
        <el-form-item label="持有天数">
          <el-input-number v-model="btForm.horizon_days" :min="5" :max="120" />
        </el-form-item>
        <el-form-item label="板块上限">
          <el-input-number v-model="btForm.max_boards" :min="1" :max="100" />
        </el-form-item>
        <el-form-item label="日期步长">
          <el-input-number v-model="btForm.date_step" :min="1" :max="20" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import rpeApi from '@/services/rpeApi'
...
async function onCreate() {
  try {
    await rpeApi.createConfig({ ...createForm })
...
async function onSaveEdit() {
  ...
  try {
    ...
    await rpeApi.updateConfig(editId.value, { config_params: params })
