/**
 * CUPB 杯底形态策略管理端 API（走 apiService，自动带 admin_token）
 */
import { apiService } from './api'

const PREFIX = '/cupb'

export type CupbScopeBody = {
  trade_date?: string
  config_id?: number
  ...
  max_results?: number
  market_scopes?: Array<'CN' | 'HK'>
  cn_board_segments?: string[]
  persist?: boolean
  force?: boolean
  price_adjust?: 'none' | 'qfq'
}

export const cupbApi = {
  listConfigs() {
  ...
  triggerPrecompute(body: CupbScopeBody) {
    return apiService.post<{
      trade_date?: string
      screened?: number
      hit_count?: number
