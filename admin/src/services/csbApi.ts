...
const PREFIX = '/api/admin/csb'

export interface CSBStrategyConfig {
  id: number
  name: string
...
class CSBApiService {
  ...
  async listStrategyConfigs(activeOnly = false): Promise<CSBStrategyConfig[]> {
    const q = activeOnly ? '?active_only=true' : ''
    const res = await this.request<{ success: boolean; data: CSBStrategyConfig[] }>(
      `${PREFIX}/strategy-configs${q}`
    )
    return res.data || []
  }

  async getStrategyConfig(id: number): Promise<CSBStrategyConfig> {
    const res = await this.request<{ success: boolean; data: CSBStrategyConfig }>(
  ...
  async createStrategyConfig(body: Partial<CSBStrategyConfig> & { name: string }) {
    const res = await this.request<{ success: boolean; data: CSBStrategyConfig }>(
...
export const csbApiService = new CSBApiService()
