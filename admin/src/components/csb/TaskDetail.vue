...
async function load() {
  if (!props.taskId || !csbApi) return
  loading.value = !task.value
  try {
    task.value = await csbApi.getBacktest(props.taskId)
    try {
      logs.value = await csbApi.getBacktestLogs(props.taskId)
    } catch {
...
function pdfFilename() {
  ...
  const namePart = rawName
    ...
    .replace(/\s+/g, '_')
    .slice(0, 40)
  const mid = namePart || `CSB回测_${id}`
  return `CSB回测详情_${mid}_${y}${m}${day}.pdf`
}

async function exportPdf() {
  if (!props.taskId || !csbApi || exporting.value) return
  exporting.value = true
  try {
    const blob = await csbApi.downloadBacktestPdf(props.taskId)
