<template>
  <div class="csb-report-analysis">
    ...
    <el-dialog v-model="detailVisible" title="CSB 报告详情" width="860px" destroy-on-close>
...
const csbApi = inject<any>('csbApi')
...
async function refresh() {
  ...
  loading.value = true
  try {
    reports.value = await csbApi.getReports({ limit: 100 })
  } catch (e: any) {
...
async function viewReport(row: any) {
  try {
    currentReport.value = await csbApi.getReport(row.report_id)
...
function downloadReport(row: any) {
  window.open(csbApi.reportDownloadUrl(row.report_id), '_blank')
}

async function deleteReport(row: any) {
  try {
    await ElMessageBox.confirm(`确认删除报告「${row.name}」？`, '提示', { type: 'warning' })
    await csbApi.deleteReport(row.report_id)
