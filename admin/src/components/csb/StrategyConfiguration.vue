<template>
  <div class="csb-strategy-configuration">
    <el-row :gutter="16">
      <el-col :span="7">
        <el-card header="CSB 参数版本">
...
const versions = ref<CSBStrategyConfig[]>([])
const listLoading = ref(false)
const selectedId = ref<number | null>(null)
const selected = ref<CSBStrategyConfig | null>(null)
...
async function loadVersions() {
  ...
  try {
    versions.value = await csbApiService.listStrategyConfigs()
...
async function onSelectVersion(row: CSBStrategyConfig | null) {
  if (!row) return
  selectedId.value = row.id
  selected.value = row
  try {
    const data = await csbApiService.getStrategyConfig(row.id)
    selected.value = data
...
async function saveVersion() {
  ...
  try {
    ...
    await csbApiService.updateStrategyConfig(selectedId.value, {
...
async function createVersion() {
  try {
    const { value } = await ElMessageBox.prompt('请输入版本名称', '新建 CSB 参数版本', {
      inputValue: `csb_${Date.now()}`,
      confirmButtonText: '创建',
      cancelButtonText: '取消',
    })
    const defaults = await csbApiService.getDefaultParams()
    const created = await csbApiService.createStrategyConfig({
      name: value,
      version_label: 'v1',
...
async function runPreview() {
  ...
  try {
    const res = await csbApiService.screenPreview({ limit: 50, config_id: selectedId.value })
