...
const AdminPanel = {
    ...
    createUserModal(title, userData = {}) {
        ...
        modal.querySelector('.modal-save').onclick = (e) => {
            e.preventDefault();
            if (isEdit) {
                this.updateUser(userData.id, modal);
            } else {
    ...
    // 验证表单数据
    validateUserForm(username, email, password, isEdit = false) {
        // 验证用户名
        ...
        if (username.trim().length < 3) {
            this.showToast('用户名长度至少3位', 'error');
            return false;
        }
        ...
        if (!emailRegex.test(email.trim())) {
            this.showToast('请输入正确的邮箱格式（例如：user@example.com）', 'error');
            return false;
        }
    ...
    async createUser(modal) {
        ...
        const password = modal.querySelector('#modalPassword').value.trim();
        const status = modal.querySelector('#modalStatus').value;

        // 表单验证
        if (!this.validateUserForm(username, email, password, false)) {
            return;
        }
        ...
        try {
            ...
            this.showToast('网络连接错误，请检查后端服务是否启动', 'error');
        }
    },
    ...
    async updateUser(userId, modal) {
        ...
        // 表单验证
        if (!this.validateUserForm(username, email, '', true)) {
            return;
        }
