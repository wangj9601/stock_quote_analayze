...
from .backtest_runner import run_csb_backtest
...
def _run_task(task_id: str) -> None:
    ...
    try:
        ...
        logger.exception("CSB 回测任务失败 %s", task_id)
...
def _maybe_start_hit_rate_compare(parent_id: str) -> Optional[str]:
    ...
    child_name = f"{parent.get('name') or 'CSB回测'}_命中率对照"
    ...
    backtest_storage.patch_task_summary(
        parent_id,
        {"paired_hit_rate_task_id": child_id, "paired_hit_rate_name": child_name},
    )
    start_backtest_task(child_id)
    logger.info("CSB 已自动创建 hit_rate 对照 parent=%s child=%s", parent_id[:8], child_id[:8])
    return child_id
