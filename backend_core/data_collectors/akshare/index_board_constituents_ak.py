"""
指数板块成分股采集：按 board_code_source 路由。

- eastmoney / tonghuashun（及空值按 LEGACY）：ak.stock_zh_index_cons
...
resolve_index_cons_fetcher = resolve_industry_cons_fetcher


class IndexBoardConstituentsCollector:
    log_table = "realtime_collect_operation_logs"

    def __init__(self) -> None:
        self.interval_sec = _env_float("INDEX_CONS_API_INTERVAL_SEC", 0.5)
        self.max_retries = int(os.getenv("INDEX_CONS_MAX_RETRIES", "2"))
    ...
    def fetch_board_constituents(self, board_code: str) -> List[Tuple[str, str]]:
        ...
        symbol = str(board_code or "").strip()
        # 部分接口对 BK 前缀不友好，纯数字优先
        if symbol.upper().startswith("BK") and symbol[2:].isdigit():
            symbol = symbol[2:]
        for attempt in range(self.max_retries + 1):
            try:
                df = ak.stock_zh_index_cons(symbol=symbol)
    ...
    def save_board_constituents(
        ...
        for stock_code, stock_name in constituents:
            session.execute(
                text(
                    """
                    INSERT INTO index_board_constituents
    ...
    def write_log(
        ...
        try:
            ...
            session.execute(
                ...
                {
                    "operation_type": "index_board_constituents",
    ...
    def run(self, board_codes: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not boards:
                print("[指数成分股] index_board_basic_info 为空，请先在管理端新增指数板块")
                self.write_log("指数成分股同步", 0, "fail", "无板块列表")
                return {
                    "ok_boards": 0,
                    "fail_boards": [],
                    "skip_boards": [],
            ...
            print(
                f"[指数成分股] 开始同步 {len(boards)} 个板块（含全部代码来源），间隔 {self.interval_sec}s"
            )
            for i, (board_code, board_name, source) in enumerate(boards, 1):
                label = board_name or board_code
                route = resolve_index_cons_fetcher(source)
                ...
                if route == "unsupported":
                    ...
                    print(f"[指数成分股] 跳过 {label} ({board_code}/{source}): {reason}")
                    continue
                try:
                    ...
                    if i % 20 == 0 or i == len(boards):
                        print(f"[指数成分股] {i}/{len(boards)} {label}[{source}] -> {n} 只")
                except Exception as e:
                    session.rollback()
                    fail_boards.append(
                    ...
                    print(f"[指数成分股] 失败 {label} ({board_code}/{source}): {e}")
            ...
            print(f"[指数成分股] 异常: {e}\n{tb}")
            self.write_log("指数成分股同步异常", 0, "fail", str(e) + "\n" + tb)
            return {
                "ok_boards": ok_boards,
                "fail_boards": fail_boards,
                "skip_boards": skip_boards,
