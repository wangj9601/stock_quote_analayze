...
def _list_observe_impl(
    ...
    if market:
        q = q.filter(TripleVolumeObserveStock.market == market.upper())
    if status:
...
def list_observe_stocks(
    ...
    return _list_observe_impl(db, market, status, page, page_size)
...
def export_observe_stocks(
    ...
    return _export_observe_impl(db, market, status, principal)
