# -*- coding: utf-8 -*-
"""CSB 通道粘合突破策略 — 管理端 API（参数版本 + 预计算 + 回测）。"""
...
router = APIRouter(prefix="/api/admin/csb", tags=["CSB Admin"])
...
class BacktestCreateBody(BaseModel):
    ...
    signal_quality_mode: Optional[str] = Field(
        "standard",
        description="信号质量: standard=标准 | premium=精选",
    )
...
def write_csb_audit(
    ...
    *,
    ...
    if not log_type.startswith("csb_"):
        log_type = f"csb_{log_type}"
    try:
        ...
        logger.warning("写入 CSB 审计日志失败: %s", e)
        try:
            db.rollback()
        except Exception:
            pass
...
def _resolve_effective_config_id(db: Session, mgr: CSBConfigManager) -> Optional[int]:
...
def _get_config_meta(db: Session, mgr: CSBConfigManager, config_id: int) -> Dict[str, Any]:
    row = mgr.get_config_row(db, config_id)
    if not row:
        return {}
    data = mgr._serialize_row(row)
    effective_id = _resolve_effective_config_id(db, mgr)
    data["config_params"] = mgr.get_config(config_id, db=db)
    data["is_effective"] = effective_id is not None and int(config_id) == int(effective_id)
    if data.get("version_label") is None and hasattr(row, "version_label"):
        data["version_label"] = getattr(row, "version_label", None)
    return data
...
def _attach_csb_trade_meta(db: Session, config: Dict[str, Any]) -> Dict[str, Any]:
    from backend_core.strategies.csb.backtest_runner import build_csb_trade_meta
...
def _build_backtest_config(db: Session, body: BacktestCreateBody) -> Dict[str, Any]:
    from backend_core.strategies.csb.backtest_runner import resolve_target_pct_range
    ...
    meta = _get_config_meta(db, mgr, int(resolved_id)) if resolved_id is not None else {}
    ...
    return _attach_csb_trade_meta(db, config)
...
async def get_system_status(db: Session = Depends(get_db)):
    try:
        ...
        logger.exception("CSB system/status 失败")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/audit-logs")
async def list_csb_audit_logs(
    limit: int = Query(50, ge=1, le=200),
    offset: int = Query(0, ge=0),
    log_type: Optional[str] = Query(None, description="如 csb_config_update"),
    db: Session = Depends(get_db),
):
    try:
        where = "WHERE log_type LIKE 'csb_%'"
        params: Dict[str, Any] = {"lim": limit, "off": offset}
        if log_type:
            where += " AND log_type = :lt"
            params["lt"] = log_type if log_type.startswith("csb_") else f"csb_{log_type}"
        rows = db.execute(
            text(
                f"""
        ...
        logger.exception("CSB audit-logs 查询失败")
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_strategy_configs(
    ...
    try:
        ...
        logger.exception("CSB strategy-configs list 失败")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_strategy_config(config_id: int, db: Session = Depends(get_db)):
    try:
        ...
        logger.exception("CSB strategy-config get 失败")
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_strategy_config(body: StrategyConfigCreateBody, db: Session = Depends(get_db)):
    try:
        ...
        logger.exception("CSB strategy-config create 失败")
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_strategy_config(
    ...
    try:
        ...
        logger.exception("CSB strategy-config update 失败")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_default_params():
    mgr = CSBConfigManager()
    return {"success": True, "data": mgr.get_default_config()}
...
async def list_watchlist_users(db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.exception("CSB watchlist-users 失败")
        raise HTTPException(status_code=500, detail=str(e))
...
async def screen_preview(
    ...
    try:
        ...
        return CSBFrontendInterface.screen(
            db,
            scope="all",
            limit=limit,
            screening_date=date,
        ...
        logger.exception("CSB screen-preview 失败")
        raise HTTPException(status_code=500, detail=str(e))
...
async def run_precompute(
    ...
    def _job():
        from backend_core.strategies.csb.scheduled_precompute import (
            list_precompute_config_ids,
            run_csb_precompute_for_config,
        )
        ...
        if config_id is not None:
            ...
            try:
                ...
                db.close()
            for cid in ids:
                run_csb_precompute_for_config(cid, trade_date=trade_date, limit=limit)

    threading.Thread(target=_job, daemon=True, name="csb-precompute-manual").start()
    return {
        "success": True,
        "message": "预计算任务已启动",
        "date": trade_date,
...
async def refresh_trace_range(body: TraceRefreshRangeBody, db: Session = Depends(get_db)):
    ...
    def _job():
        from backend_core.strategies.csb.scheduled_precompute import run_csb_trace_refresh_range

        run_csb_trace_refresh_range(
            cid,
            start_date=start_s,
            end_date=end_s,
    ...
    threading.Thread(
        target=_job,
        daemon=True,
        name=f"csb-trace-refresh-{cid}",
    ).start()
    write_csb_audit(
        db,
        "csb_trace_refresh_range",
        {
            "config_id": cid,
            "start_date": start_s,
            "end_date": end_s,
...
async def get_backtest(task_id: str, db: Session = Depends(get_db)):
    ...
    if not cfg.get("trade_logic") or not cfg.get("risk_params"):
        try:
            patched = _attach_csb_trade_meta(db, dict(cfg))
            cfg = {**cfg, "trade_logic": patched.get("trade_logic"), "risk_params": patched.get("risk_params")}
            row = {**row, "config": cfg}
        except Exception:
            logger.exception("CSB 回测详情补齐交易逻辑失败 task=%s", task_id)
...
async def export_backtest_pdf(task_id: str, db: Session = Depends(get_db)):
    ...
    from backend_core.strategies.csb import backtest_storage
    from backend_core.strategies.csb.backtest_pdf import render_csb_backtest_pdf
    ...
    if not cfg.get("trade_logic") or not cfg.get("risk_params"):
        try:
            ...
            logger.exception("CSB PDF 导出补齐交易逻辑失败 task=%s", task_id)
    if summary and (not summary.get("trade_logic") or not summary.get("risk_params")):
        row = {
            **row,
            "summary": {
    ...
    try:
        pdf_bytes = render_csb_backtest_pdf(row)
        if not pdf_bytes:
            raise RuntimeError("xhtml2pdf 未安装，无法导出 PDF")
        ...
        logger.exception("CSB 回测 PDF 导出失败 task=%s", task_id)
        raise HTTPException(status_code=500, detail=f"导出PDF失败: {e}") from e

    short = (task_id or "")[:8] or "unknown"
    ascii_filename = f"csb_backtest_{short}.pdf"
    utf8_filename = quote(f"CSB回测详情_{short}.pdf")
    return Response(
        content=pdf_bytes,
...
async def export_backtest(task_id: str):
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="csb_backtest_{task_id[:8]}.csv"'},
    )
...
async def export_backtest_xlsx(task_id: str):
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="csb_backtest_{task_id[:8]}.xlsx"'},
    )
...
async def download_report(report_id: str):
    ...
    return Response(
        content=raw,
        media_type="text/csv; charset=utf-8",
        headers={"Content-Disposition": f'attachment; filename="csb_report_{report_id[:8]}.csv"'},
    )
...
async def download_report_xlsx(report_id: str):
    ...
    return Response(
        content=raw,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f'attachment; filename="csb_report_{report_id[:8]}.xlsx"'},
    )
