...
OWN-WORLD: 哑光深底 `#0f1419` + 地图灰墨 `#d9e2ec`；叠层金 `#c9a227`、航线蓝 `#2f6fed`、警戒红 `#c23b3b`。半透明叠层芯片、极淡暗角底图（无满屏方格）、字号阶梯导航；无玻璃拟态、无霓虹描边。

STORY: 进站即认冰枫堂与三大频道；打开策略叠层看 GMS/RPE/URT 等印在底图上的信号；进选股/个股仍是同一套叠层语法。

FIRST VIEWPORT: 顶栏左冰枫堂字标，频道名（自选/行情/选股）用跨房可读大字号；其下一条策略叠层开关条；主区为板块/行情抬升工作面，右侧信号/龙头中军对向力侧栏。登录页为同一地图的图例入口，不另起营销站。
