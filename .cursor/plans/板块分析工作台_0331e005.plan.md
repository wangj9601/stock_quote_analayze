...
#### 3.2.1 支撑/压力价位源（含 Fib / Pivot / Volume Profile）
...
| 动态均线 | 已有 | 旁证 |
| **Fibonacci** | 已落地 | 参考支撑/压力价 |
| **经典 Pivot** | 已落地 | 参考支撑/压力价 |
| **日线 Volume Profile** | **已落地（轻量）** | POC/VAH/VAL 与 KDE 对照；不改策略门槛 |
...
4. **Volume Profile** → 仅「我的→支撑压力」与 KDE 并列展示对比（`volume_profile` + `vp_vs_kde`）；**不写入策略硬筛 / trade_advice 门槛**

#### 3.2.1b 日线 Volume Profile 口径（固化）

模块：[`backend_core/analysis/volume_profile.py`](backend_core/analysis/volume_profile.py)

- **输入**：与 Fib 同窗默认 `lookback=60` 日 K 的 high/low/close/volume；前复权计算时用同一套复权 OHLC（volume 不复权）。
- **分桶**：窗口最低价～最高价均分为约 24 桶（8～48）；每日成交量在当日 [low,high] 覆盖的桶上**均匀分摊**。
- **POC**：成交量最大桶的中点价。
- **价值区**：自 POC 向两侧扩展，直至累计成交量 ≥ 总成交量的 **70%** → `VAL`（下沿）/ `VAH`（上沿）。
- **最近支撑/压力**：现价下方取 VAL/POC 中最近；上方取 VAH/POC 中最近。
- **与 KDE 对比**：`vp_vs_kde` 比较最近支撑/压力绝对差与相对差；相对偏差 ≤1.5% 标「共振」。
- **角色**：辅助对照 KDE；短线仍以 KDE 为主。不改 GMS/URT/SBBR/RPE 引擎。

#### 3.2.2 Fibonacci / Pivot / 共振带算法口径（已升级）

模块：[`classic_levels.py`](backend_core/analysis/classic_levels.py) + [`swing_zigzag.py`](backend_core/analysis/swing_zigzag.py) + [`pivot_variants.py`](backend_core/analysis/pivot_variants.py) + [`confluence_zones.py`](backend_core/analysis/confluence_zones.py)。

**数据输入（日线）：**

- 批量/Fib 回看默认 **180** 根；VP 仍内部截断 **60**。
- **Fib 摆动锚点**：Williams 分形预选 + ZigZag 深度过滤（`depth=max(2.5%*close, 1.5*ATR14)`）；不足时补窗口绝对高低；只用最近一对已确认 swing；`anchor_method=zigzag_fractal`。
- 区间过窄（`(H-L)/L < 1%`）则跳过 Fib 回撤位，仍返回锚点元数据。
...
**Camarilla / ATR-Pivot（波动率修正，参考用）：** 上一交易日 H/L/C 算 Camarilla R1–R4/S1–S4；Wilder ATR14 得 `P±1/2ATR` 带。短线 UI 优先展示 Cam 最近支撑/压力；ATR 带进共振候选。

**共振带 confluence_zones：** KDE/VP/Fib/Cam/经典 Pivot/ATR 加权候选 → 1D DBSCAN（无 sklearn 则距离合并）→ 支撑/压力各最多 3 带。`trade_advice` 仅软融合（summary / 展示价 `basis=kde+confluence`），**不改**四策略硬门槛。

**批量性能：** 聚合接口对命中股批量拉 K 线（按 code 列表一次查询 lookback 窗口），内存算 Fib/Pivot/共振，避免 N+1。

**角色：** 「买卖判断参考」；UI 标签「参考价 Fib/Cam/VP/合」；不改 GMS/URT/SBBR/RPE 引擎门槛。
...
- 调用聚合 API；四策略结果分区表格；列：代码、名称、角色、命中标签、主买点/主卖点（策略+KDE）、**参考价 Fib/Cam/VP/合**、操作。
- 参考价列展示最近 Fib / Camarilla / VP / 共振带；悬停含 ZigZag 锚点、Cam 全档、ATR 带、共振带明细。
...
- ~~不做 Camarilla~~ → 已落地 Camarilla + ATR-Pivot + 共振带（仍不做 Woodie 全套替换）。
- 不在本阶段做全市场多板批量扫描看板。
- 不把共振带 / Camarilla 做成选股硬过滤；不做实时分笔 ZigZag。
