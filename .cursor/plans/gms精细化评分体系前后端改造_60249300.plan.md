...
overview: 依据《交易评分体系细化手册-20260201》和《均值引力与动量突变策略实战执行手册-20260201》，将 GMS 策略从单一混合评分改为双模块阶梯式评分（吸附态 / 突变态），并同步前后端展示与参数。
todos:
...
| 维度   | 现状                         | 新规                                            |
| ---- | -------------------------- | --------------------------------------------- |
| 评分结构 | 单一百分制：蓄势(30)+平衡(40)+动量(30) | 双模块各100分：吸附态 / 突变态                            |
| 吸附态  | 简单阈值                       | 阶梯式：F/Z、                                      |
| 突变态  | 简单阈值                       | 阶梯式：Δ/d₁、d₂₀-d、m₂₀/m 各档位                      |
| 执行标准 | 总分>60/90                   | 吸附态≥85 S级、[70,85) A级；突变态≥90 全速切入、[80,90) 分批买入 |
...
flowchart TB
    ...
    subgraph CalcLayer [计算层]
        ...
        Calc --> AccScore[吸附态阶梯评分]
        Calc --> MomScore[突变态阶梯评分]
...
- `score_accumulation`：吸附态总分 0–100
- `score_momentum`：突变态总分 0–100（可含负分）
...
- 吸附态：`accumulation_fz_tiers`、`balance_ratio_d_tiers`、`volume_ratio_shrink_tiers`
- 突变态：`ratio_d1_tiers`、`instant_deviation_stable_days`(3)、`volume_ratio_attack_tiers`
...
- 左侧买点：优先用吸附态得分与 S/A 级判断。
- 右侧买点：优先用突变态得分与全速/分批判断。
...
- 新增可选 Query：吸附态/突变态各阶梯阈值、执行标准阈值，用于覆盖 config。
- 与现有一致：以前端传入参数为准。
...
- 将单一百分制说明改为双模块说明。
- 吸附态：F/Z、Δ/d、m₂₀/m 阶梯规则及 S/A 级标准。
- 突变态：Δ/d₁、d₂₀-d、m₂₀/m 阶梯规则及全速切入/分批买入标准。
...
- 吸附态：F/Z 档位（2.5/1.5）、Δ/d 档位（1%/1.5%）、地量档位（0.6/0.8）、S/A 阈值（85/70）。
- 突变态：量比档位（2.0/1.5）、推力支撑“站稳3日”开关、全速/分批阈值（90/80）。
...
- 拆分为两个表格：「吸附态得分」与「突变态得分」。
- 吸附态：时间耗散(F/Z)、引力粘合(Δ/d)、成交量缩(m₂₀/m)、小计、等级(S/A)。
- 突变态：盈亏反转(Δ/d₁)、推力支撑(d₂₀-d)、攻击强度(m₂₀/m)、小计、等级(全速/分批)。
- 显示各档位阈值及阶梯规则说明。

### 4.4 结果表格

- 列扩展：吸附态得分、吸附态等级、突变态得分、突变态等级（可选列或折叠展示）。
