            ...
            if (data.success) {
                ...
                if (currentSubIndicator === 'macd') {
                    ...
                    console.log('[loadKlineData] 准备显示MACD图表，检查数据:', {
                        hasDIF: option.series[9] && option.series[9].data && option.series[9].data.length > 0,
                        hasDEA: option.series[10] && option.series[10].data && option.series[10].data.length > 0,
                        hasMACD: option.series[11] && option.series[11].data && option.series[11].data.length > 0,
                        difDataLength: option.series[9] && option.series[9].data ? option.series[9].data.length : 0,
                        deaDataLength: option.series[10] && option.series[10].data ? option.series[10].data.length : 0,
                        macdDataLength: option.series[11] && option.series[11].data ? option.series[11].data.length : 0,
                        macdValidCount: option.series[11] && option.series[11].data ? option.series[11].data.filter(d => d !== null && !isNaN(d)).length : 0,
                        sampleDIF: option.series[9] && option.series[9].data ? option.series[9].data.slice(0, 3) : [],
                        sampleDEA: option.series[10] && option.series[10].data ? option.series[10].data.slice(0, 3) : [],
                        sampleMACD: option.series[11] && option.series[11].data ? option.series[11].data.slice(0, 3) : []
                    });
                    // 注意：必须先隐藏其他指标，再显示MACD，避免grid被重置
                    this.hideVolumeChart(option);
                    this.hideKDJChart(option);
